...
audio_schema.pre("save", async function (next) {
  ...
  try {
    const the_user = await UserModel.findOne({
    ...
    if (the_user) {
      ...
      console.error(
        `User not found for wallet_address: ${author_wallet_address}`
      );
    }
    next();
  } catch (err: any) {
    console.log("DB Error Uploading Audio.", err);
    next(err);
  }
});
...
const AudioModel =
  mongoose.models.Audio || mongoose.model<IAudio>("Audio", audio_schema);
