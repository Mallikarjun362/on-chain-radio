...
import AudioRoomModel from "@/models/AudioRoom";
...
export async function uploadAudio({
  ...
  try {
    ...
    console.log("Uploading Audio to Database (Success).")
...
export async function getAllActiveRooms(): Promise<
  Array<typeof AudioRoomModel>
> {
...
export async function setRoomActive(roomId: string) {
  const the_audio_room = await AudioRoomModel.findByIdAndUpdate(roomId, {
    is_active: true,
  });
  return true;
}
...
export async function createRoom(
  ...
  if (!wallet_address) {
    ...
    const new_audio_room = new AudioRoomModel({
...
export async function getRoomDetailsById(roomId: string) {
  ...
  const result = await AudioRoomModel.findById(roomId)
    .select("main_author_wallet_address description title")
    .exec();
  return JSON.parse(JSON.stringify(result));
}

export async function getRoomsOfAnUser(
  wallet_address: string
): Promise<Array<any>> {
  console.log("Function: Get Rooms of An User");
  const results = await AudioRoomModel.find({
    main_author_wallet_address: wallet_address,
  });
  return JSON.parse(JSON.stringify(results));
}
